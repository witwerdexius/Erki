...
import { X, Search, Plus, Pencil, Trash2, ArrowLeft, Check, Clock, Users } from 'lucide-react';
import type { TaskSection, TaskTemplate } from '@/lib/types';
...
interface Props {
  ...
  onClose: () => void;
  onCreateTemplate: () => Promise<TaskTemplate | null>;
  onSaveTemplate: (id: string, data: Partial<TaskTemplate>) => Promise<void>;
  onDeleteTemplate: (id: string, name: string) => Promise<void>;
}

export default function TaskTemplatePickerDialog({
  sectionLabel,
  templates,
  onSelect,
  onClose,
  ...
  const [editingData, setEditingData] = useState<Partial<TaskTemplate>>({});
  ...
  const handleSave = async () => {
    if (!editingId) return;
    await onSaveTemplate(editingId, editingData);
    setEditingId(null);
    setEditingData({});
  };
  ...
  const addLabel = selectedIds.size === 0
    ...
    : selectedIds.size === 1
      ...
      : `${selectedIds.size} Aufgaben hinzufügen`;
        ...
        {editingId ? (
            ...
            <div className="overflow-y-auto flex-1 px-5 py-4 space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  value={editingData.helpersRequired ?? 1}
                  onChange={(e) => setEditingData(d => ({ ...d, helpersRequired: Math.max(1, parseInt(e.target.value) || 1) }))}
                  onFocus={e => e.target.select()}
                  className="w-16 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#6bbfd4]/30"
                />
                <label className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1 shrink-0">
                ...
                <input
                  ...
                  onChange={(e) => setEditingData(d => ({ ...d, time: e.target.value || undefined }))}
                  className="w-32 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#6bbfd4]/30"
                />
              </div>
            </div>
          ...
          <>
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
              <div>
                <h2 className="font-semibold text-gray-900 dark:text-gray-50 leading-tight">Vorlage auswählen</h2>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Rubrik: {sectionLabel}</p>
              </div>
                ...
                filtered.map(t => {
                  ...
                  return (
                    <div
                      key={t.id}
                      className={cn(
                        'flex items-center px-4 py-3 transition-colors border-b border-gray-100 dark:border-gray-700 last:border-0',
                        isSelected
                    ...
                    >
                      ...
                      <button
                        ...
                        className={cn(
                          'shrink-0 mr-3 w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors',
                          isSelected
                      ...
                      >
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                          {t.helpersRequired} Helfer{t.helpersRequired !== 1 ? '' : ''}
                          {t.time ? ` · ${t.time} Uhr` : ''}
                        </p>
                      </button>
                      ...
                      <div className="flex items-center gap-1 shrink-0 ml-2">
                        <button
                          onClick={() => { setEditingId(t.id); setEditingData(t); }}
