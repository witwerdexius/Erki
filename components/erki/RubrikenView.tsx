...
export default function RubrikenView({
  ...
  // Greedy column distribution: assign each section to the shortest column
  const BASE_H = 80;
  const ITEM_H = 60;
  const cols = Math.max(1, colCount);
  const masonryColumns: Array<typeof allSections> = Array.from({ length: cols }, () => []);
  const colHeights = new Array(cols).fill(0);
  for (const section of visibleSections) {
    const itemCount = section.isStationen
      ? stationCount
      : filteredTasks.filter(t => t.section === section.id).length;
    const h = BASE_H + itemCount * ITEM_H;
    const minIdx = colHeights.indexOf(Math.min(...colHeights));
    masonryColumns[minIdx].push(section);
    colHeights[minIdx] += h;
  }
  ...
  // Bottom bar (shared between mobile and desktop)
  const bottomBar = (
  ...
  return (
    <div className="flex-1 overflow-auto bg-gray-100 dark:bg-gray-800">
      ...
      {colCount <= 1 ? (
        /* Mobile: single column, normal stacking */
        <>
        ...
        /* Desktop: JS greedy masonry — flex row of flex-col columns */
        <>
