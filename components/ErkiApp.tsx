...
import { ChevronLeft, Plus, Trash2, List, Download, Upload, Link, Loader2, BookOpen, FileText, Map as MapIcon } from 'lucide-react';
...
export default function ErkiApp({ plan, user, onPlanUpdate, onExternalPlanUpdate, onSaveNow, onBack, onImmediateSave, isSaving = false, latestPlanRef, isDirtyRef }: ErkiAppProps) {
    ...
    const [activeTab, setActiveTab] = useState<'map' | 'table' | 'nachdenk' | 'explanation'>(() => {
        const stored = sessionStorage.getItem(tabKey);
        const valid: string[] = ['map', 'table', 'nachdenk', 'explanation'];
        return (valid.includes(stored ?? '') ? stored : 'table') as 'map' | 'table' | 'nachdenk' | 'explanation';
    });
