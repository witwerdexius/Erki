    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b dark:border-gray-700">
          <div>
            <h2 className="font-semibold text-gray-900 dark:text-gray-50">Verlauf</h2>
            <p className="text-xs text-gray-600 dark:text-gray-400 mt-0.5 truncate max-w-[340px]">{planningTitle}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            <p className="text-center text-sm text-gray-600 dark:text-gray-400 py-10">Wird geladen…</p>
          ) : errorMsg ? (
            <p className="text-center text-sm text-red-500 py-10">{errorMsg}</p>
          ) : snapshots.length === 0 ? (
            <p className="text-center text-sm text-gray-600 dark:text-gray-400 py-10">Keine Snapshots vorhanden.</p>
          ) : (
            <div>
              ...
              {snapshots.map(snap => (
                <div key={snap.id} className="flex items-center gap-3 px-5 py-3 border-b dark:border-gray-700 last:border-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-50">{formatDate(snap.createdAt)}</p>
                    ...
                    {confirmId === snap.id ? (
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          disabled={restoring}
                          className="text-xs px-2 py-1 border dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300 disabled:opacity-40"
                        >
                          Abbrechen
                        </button>
                      </div>
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs px-3 py-1.5 border dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300"
                      >
